...
DEFAULT_COLOR_ON = Qt.GlobalColor.red
COLOR_OFF = Qt.GlobalColor.white
...
class SevenSegmentDisplayDigit(QWidget):
    def __init__(self):
        super().__init__()
        self.value = 0
        self.color = DEFAULT_COLOR_ON
    ...
    def set_value(self, value: int | None):
        if self.value is not None and self.value not in SEGMENT_MAP:
            raise ValueError(f"Unsupported value: {self.value}")
        self.value = value
        self._update_color_from_value()

    def set_color(self, color: QColor) -> None:
        self.color = color
        self._update_color_from_value()

    def _update_colors(self, segment: Segment, color: QBrush | QColor | Qt.GlobalColor | QGradient):
    ...
    def _update_color_from_value(self):
        if self.value is None:
            for segment in Segment:
                self._update_colors(segment, COLOR_OFF)
        else:
            segments = SEGMENT_MAP[self.value]
            for segment in Segment:
                color = self.color if segment in segments else COLOR_OFF
                self._update_colors(segment, color)
